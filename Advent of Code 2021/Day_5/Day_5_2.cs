        ...
        public static int Solve()
        {
            ...
            foreach (List<int> item in lines)
            {
                ...
                else if (item[3] == 0)
                {
                    ...
                    {
                        for (int i = item[2]; i < 0 + 1; i++)
                        {
                            grid[item[1]][item[0] + i] += 1;
                        }
                    }
                    
                } else if(item[3] != 0 && item[2] != 0)
                {
                    int x = item[0];
                    int y = item[1];

                    int xDiff = item[2] / Math.Abs(item[2]);
                    int yDiff = item[3] / Math.Abs(item[3]);

                    for (int i = 0; i < Math.Abs(item[2]) + 1; i++)
                    {
                        grid[y + (yDiff * i)][x + (xDiff * i)] += 1;
                    }
                }
            }
